...
bool GpuCopyTest() {
  ...
  // 获取图像尺寸
  size_t imageSize = srcImage.total() * srcImage.elemSize();

  // 分配 GPU 内存
  unsigned char* gpuData;
  cudaMalloc(&gpuData, imageSize);

  // 将图像数据从 CPU 拷贝到 GPU
  cudaMemcpy(gpuData, srcImage.data, imageSize, cudaMemcpyHostToDevice);

  // 分配 CPU 内存
  cv::Mat cpuImage(srcImage.size(), srcImage.type());

  // 将图像数据从 GPU 拷贝到 CPU
  cudaMemcpy(cpuImage.data, gpuData, imageSize, cudaMemcpyDeviceToHost);
...
//           colors_text_[currC].c_str(), curH, curS, curV, ((int)curS - (int)preS) / float((int)preV -
//           (int)curV));
...
//   cv::glob("/home/crrcdt123/pytorch_test/pytorch-deeplab-xception/traindata/train_voc/JPEGImages/*.jpg",
//   image_paths); for (size_t idx = 0; idx < image_paths.size(); idx+=10) {
//     // std::string image_path =
//     "/home/crrcdt123/pytorch_test/pytorch-deeplab-xception/traindata/train_voc/JPEGImages/frame_002662++frame_002673.jpg";
...
//     "/home/crrcdt123/pytorch_test/pytorch-deeplab-xception/traindata/train_voc/SegmentationClass"
//      + name + ".png";
