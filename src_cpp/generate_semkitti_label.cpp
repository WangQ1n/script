...
int main() {
  std::vector<cv::String> json_dirs = {""};
  std::vector<cv::String> cloud_dirs = {""};
  std::vector<cv::String> json_paths;
  cv::glob(json_dirs[0], json_paths);
  std::cout << "总计点云帧数：" << data_paths.size() << std::endl;
  for (size_t idx = 0; idx < data_paths.size(); idx += 10) {
    ...
    // std::vector<double> rpy = {0.0191982, -0.0017608, 0.0000339}; // s8
    // std::vector<double> t = {3.76, -0.60, 0.0}; //s8
    Eigen::Isometry3f lidar_T = GetT(rpy, t);
    pcl::transformPointCloud(*src_cloud, *src_cloud, lidar_T);
    ...
    // cv::imwrite("/media/crrcdt123/glam/crrc/datasets/s8/lidar/images_test/" + fileName + ".jpg", bev_image);
    // pcl::io::savePCDFileASCII("/home/crrcdt123/output_cloud.pcd", *clouds);
  }
}
